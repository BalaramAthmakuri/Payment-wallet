...
$query1 = "SELECT BALANCE FROM bankbalance WHERE ACCOUNT_NUMBER = '$acno' ";
$rs1 = $conn->query($query1);
$rs3 = $conn->query($query1);
$rs4 = $conn->query($query1);
$resultAll1 = mysqli_query($conn, "SELECT * FROM bankbalance WHERE ACCOUNT_NUMBER = '$acno'");
if(!$resultAll1)
{
    die(mysqli_error($conn));
}
...
} else
{
    ...
    if($PIN == $rs2)
    {
        ...
        if( $amts > $rs)
        {
            ...
            $sql = "UPDATE bankbalance SET BALANCE='$rs3' WHERE ACCOUNT_NUMBER = '$acno'";
            $sql1 = "UPDATE bankbalance SET BALANCE='$rs4' WHERE PHONE_NUMBER = '$myno'";
            if ($conn->query($sql) === TRUE && $conn->query($sql1) === TRUE) 
            {
                include 'testtobank.html';
            } else {
            ...
            $trans = "SELECT PHONE_NUMBER FROM bankbalance WHERE ACCOUNT_NUMBER = '$acno' ";
            $tra = $conn->query($trans);
            $resultAll45 = mysqli_query($conn, "SELECT * FROM bankbalance WHERE ACCOUNT_NUMBER = '$acno'");
            if(!$resultAll45)
            {
                die(mysqli_error($conn));
            }
